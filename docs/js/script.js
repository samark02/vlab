...
function AddingToArray() {
    if (pcheck == false) {
        ...
        document.getElementById('add').style.display = 'block';
        setTimeout(timer, 2000);
        // console.log(ibXarray);
        // console.log(ibYarray);
        addobservation();
    }
    // if (pcheck == false) {
    // if (pcheck == true) {
...
function IBgraph() {
    ...
    //         },
    //         scales: {
    //             yAxes: [{ ticks: { min: 2.0, max: 10.0 }, scaleLabel: { display: true, labelString: "Magnetic Field (mT)" } }],
...
// remove IBgraph button
// add array to ib graph to plot it dynamically